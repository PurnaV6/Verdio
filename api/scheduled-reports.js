...
export default async function handler(req,res){
  ...
  for(const schedule of schedules){
    ...
    const sent=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${resendKey}`,'Content-Type':'application/json'},body:JSON.stringify({from,to:[schedule.recipient_email],subject:`Verd.io executive brief — ${schedule.dataset_key}`,html:`<h1>Verd.io executive brief</h1><p>Dataset: <strong>${escapeHtml(schedule.dataset_key)}</strong></p><p>Business health: <strong>${snapshot.health??'—'}/100</strong> · Data quality: <strong>${snapshot.quality??'—'}/100</strong></p><h2>Priority risks</h2><ul>${risks||'<li>No material risks recorded.</li>'}</ul><h2>Recommended actions</h2><ul>${actions||'<li>No actions recorded.</li>'}</ul><p>This automated brief is based on the latest saved Verd.io snapshot.</p>`})});
