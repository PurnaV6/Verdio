export default async function handler(req,res){
  ...
  const origin=req.headers.origin||`https://${req.headers.host}`;const link=`${origin}/?invite=${invitation.token}`;const sent=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${resendKey}`,'Content-Type':'application/json'},body:JSON.stringify({from,to:[invitation.email],subject:`You’re invited to ${invitation.organizations?.name||'Verd.io'}`,html:`<h1>Join ${invitation.organizations?.name||'the Verd.io workspace'}</h1><p>You have been invited as <strong>${invitation.role}</strong>.</p><p><a href="${link}">Accept invitation</a></p><p>This invitation expires ${new Date(invitation.expires_at).toUTCString()}.</p>`})});
