...
<head>
<meta charset="utf-8" />
<title>Verd.io Executive Decision Report - ${result.source.fileName}</title>
<style>
...
<body>
  <div class="header">
    <div>
      <div style="font-weight:900; font-size:22px; letter-spacing:-0.02em;">Verd.io</div>
  ...
  ${vde ? `
  <div class="section">
    <h2>Verd.io Decision Engine v2 — Financial Ranking</h2>
  ...
  <div class="footer">
    Generated by Verd.io Adaptive Intelligence on ${date}. This report is derived from ${result.source.rowCount.toLocaleString()} primary-source rows across ${result.profile.columnCount} columns${result.organization ? ` within an organisational workspace containing ${result.organization.datasets.length} datasets and ${result.organization.relationships.filter((r:any)=>r.confirmed).length} confirmed relationships` : ''}. Semantic confidence and capability gating ensure only mathematically valid analyses are included. Model selection and financial impact are estimates to support decision making, not financial advice.
  </div>
...
export function emailExecutiveSummary(result: any): void {
  ...
  const subject = `Verd.io executive summary — ${result.source.fileName}`;
  const body = [
    `Verd.io executive summary for ${result.source.fileName}`,
    '',
    ...
    '',
    'Open Verd.io to review the supporting evidence and full decision report.',
  ].join('\n');
