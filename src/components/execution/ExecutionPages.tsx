...
export function PageActionTracker({ r }: { r: PipelineResult }) {
  ...
  return <div className="space-y-5"><SectionHeader eyebrow="DECISION EXECUTION" title="Action tracker" description="Convert Verd.io recommendations into accountable work with clear ownership, deadlines and delivery status."/>
...
export function PageKpiTargets({ r }: { r: PipelineResult }) {
  ...
  return <div className="space-y-5"><SectionHeader eyebrow="PERFORMANCE MANAGEMENT" title="KPI targets" description="Set an operating ambition against the current baseline and focus leadership attention on measurable gaps."/>
    ...
    <div className="configuration-note"><Target size={17}/><div><strong>Targets are planning controls · {mode === 'cloud' ? 'Cloud saved' : mode === 'syncing' ? 'Synchronising' : 'Local fallback'}</strong><p>Current values come from Verd.io's analysis; targets are leadership inputs and do not alter forecasts, health scoring or source data.</p></div></div>
  </div>;
}
