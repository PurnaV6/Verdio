...
import { AlertTriangle, Bell, CheckCircle, Clock, Database, FileSpreadsheet, Lightbulb, Link2, LockKeyhole, Network, ShieldCheck, SlidersHorizontal, Trash2, TrendingUp } from 'lucide-react';
...
export function PageRelationships({ r }: { r: PipelineResult }) {
  ...
  return <div className="space-y-5"><SectionHeader eyebrow="ORGANISATIONAL MODEL" title="Dataset relationships" description="Review connected sources, governed relationships and the operational signals supported by them."/><div className="relationship-summary-grid">{organization.datasets.map(dataset=><article key={dataset.id} className={dataset.primary?'is-primary':''}><span>{dataset.purpose}</span><strong>{dataset.fileName}</strong><p>{dataset.rowCount.toLocaleString()} rows · {dataset.columnCount} columns</p>{dataset.primary&&<small>Primary executive analysis</small>}</article>)}</div>{metrics.length>0&&<section className="connected-intelligence"><div className="relationship-title"><div><TrendingUp size={17}/><span><strong>Cross-dataset indicators</strong><small>Calculated from the uploaded files; evidence is shown for every value.</small></span></div><b>{metrics.length} indicators</b></div><div className="connected-metrics">{metrics.map(metric=><article key={metric.id}><span>{metric.label}</span><strong>{formatMetric(metric.value,metric.format)}</strong><p>{metric.evidence}</p></article>)}</div></section>}{insights.length>0&&<section className="connected-recommendations"><div className="relationship-title"><div><Lightbulb size={17}/><span><strong>Connected-data recommendations</strong><small>Actions are qualified by relationship confidence and data coverage.</small></span></div></div>{insights.map(insight=><article key={insight.id} className={`is-${insight.priority}`}><span>{insight.priority==='high'?<AlertTriangle size={17}/>:<Lightbulb size={17}/>}</span><div><strong>{insight.title}</strong><p>{insight.description}</p><small><b>Recommended next step:</b> {insight.action}</small></div><em>{Math.round(insight.confidence*100)}% evidence confidence</em></article>)}</section>}<div className="relationship-map"><div className="relationship-title"><div><Network size={17}/><span><strong>Confirmed relationship map</strong><small>{confirmed.length} governed connections across {organization.datasets.length} datasets</small></span></div></div>{confirmed.length===0?<div className="relationship-empty">No relationships were confirmed for this workspace.</div>:confirmed.map(relation=>{const left=organization.datasets.find(item=>item.id===relation.leftDatasetId);const right=organization.datasets.find(item=>item.id===relation.rightDatasetId);return <div className="relationship-map-row" key={relation.id}><div><strong>{left?.fileName}</strong><small>{relation.leftColumn}</small></div><span><Network size={14}/><b>{Math.round(relation.confidence*100)}%</b></span><div><strong>{right?.fileName}</strong><small>{relation.rightColumn} · {relation.overlapPct}% overlap</small></div></div>})}</div></div>;
}
...
export function PageAlerts({ r }: { r: PipelineResult }) {
  ...
  ].map(rule=><div className="setting-row" key={rule.key}><label><input type="checkbox" checked={prefs[rule.key]} onChange={e=>update(rule.key,e.target.checked)}/><span><strong>{rule.label}</strong><small>{rule.desc}</small></span></label>{rule.threshold&&<div className="threshold-input"><label className="sr-only" htmlFor={`${rule.key}-threshold`}>{rule.label} threshold</label><input id={`${rule.key}-threshold`} type="number" min="1" max="100" value={prefs[rule.threshold]} onChange={e=>update(rule.threshold,Number(e.target.value))}/><span>/100</span></div>}</div>)}<h2 className="settings-subheading">Scheduled executive report</h2><div className="schedule-grid"><label><span>Cadence</span><select value={prefs.reportCadence} onChange={e=>update('reportCadence',e.target.value as OperationalPreferences['reportCadence'])}><option value="off">Off</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option></select></label><label><span>Delivery email</span><input type="email" value={prefs.reportEmail} onChange={e=>update('reportEmail',e.target.value)} placeholder="executive@company.com"/></label></div><div className="settings-footer"><p><Clock size={13}/> Schedules are stored securely in Supabase and delivered by the scheduled-reports service.</p><button onClick={persist}>{saved?'Cloud schedule saved':'Save schedule'}</button></div></div></div>;
}
